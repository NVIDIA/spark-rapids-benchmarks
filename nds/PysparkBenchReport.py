...
def main():
    if len(sys.argv) != 2:
        print("Usage: python PysparkBenchReport.py <listener>")
        sys.exit(1)

    listener = PythonListener(sys.argv[1])
    report = PysparkBenchReport(listener)

    task_failures = report.get_task_failures()
    final_plan = report.get_final_plan()
    report.reset()

    print("Task Failures:")
    print(json.dumps(task_failures, indent=4))
    print("\nFinal Plan:")
